#!/usr/bin/env python3
"""Ajoute les cartes météo J/J+1 et potentiel thermique à l'accueil PCHA.

Migration volontairement stricte et idempotente : aucune modification n'est faite
si la structure attendue du dashboard n'est pas trouvée exactement une fois.
"""

from pathlib import Path

DASHBOARD = Path("dashboard/piscine.yaml")
MARKER_V3 = "# Météo et potentiel thermique V3"


def replace_once(text: str, old: str, new: str, label: str) -> str:
    count = text.count(old)
    if count != 1:
        raise SystemExit(f"ERREUR — {label}: {count} occurrence(s), 1 attendue. Aucun fichier écrit.")
...
def main() -> None:
    ...
    if MARKER_V3 in text:
        print("Dashboard météo V3 déjà appliqué — aucune modification.")
        return
    ...
    if text.count(marker) != 1:
        raise SystemExit("ERREUR — marqueur Réglages rapides absent ou ambigu. Aucun fichier écrit.")
        ...
        custom_fields:
          contenu: |
            [[[
              ...
              const tu=a.temperature_unit||'°C'; const wu=a.wind_speed_unit||'km/h';
              return `<div class="pcha-wx"><div class="pcha-wx-head"><ha-icon icon="${icon}"></ha-icon><span>Aujourd’hui</span></div><div class="pcha-wx-main"><strong>${condition}</strong><b>${f(a.temperature_min)} ${tu} <span>→</span> ${f(a.temperature_max)} ${tu}</b></div><div class="pcha-wx-meta"><span><ha-icon icon="mdi:water-percent"></ha-icon>${f(a.humidity,0)} %</span><span><ha-icon icon="mdi:weather-windy"></ha-icon>${f(a.wind_speed)} ${wu}</span></div></div>`;
            ]]]
        ...
        custom_fields:
          contenu: |
            [[[
              ...
              const icon=icons[entity?.state]??'mdi:weather-partly-cloudy';
              const tu=a.temperature_unit||'°C'; const wu=a.wind_speed_unit||'km/h';
              return `<div class="pcha-wx"><div class="pcha-wx-head"><ha-icon icon="${icon}"></ha-icon><span>Demain</span></div><div class="pcha-wx-main"><strong>${condition}</strong><b>${f(a.temperature_min)} ${tu} <span>→</span> ${f(a.temperature_max)} ${tu}</b></div><div class="pcha-wx-meta"><span><ha-icon icon="mdi:water-percent"></ha-icon>${f(a.humidity,0)} %</span><span><ha-icon icon="mdi:weather-windy"></ha-icon>${f(a.wind_speed)} ${wu}</span></div></div>`;
            ]]]
        styles: *meteo_card_styles
        extra_styles: |
          .pcha-wx { display:grid; gap:10px; width:100%; }
...
'''
    ...
    if text == original:
        raise SystemExit("ERREUR — aucune modification produite.")

    DASHBOARD.write_text(text, encoding="utf-8")
    print("OK — cartes Météo aujourd’hui, Demain et Potentiel thermique ajoutées.")
